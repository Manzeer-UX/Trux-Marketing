...
This final override supersedes the earlier fixed content-container guidance, static social controls, non-interactive FAQ behavior, and the statement that the page has no Client Component boundary. Matching the 1512px-wide Figma frame, every top-level section and its layout wrapper span the full available viewport width; responsive alignment comes from 24px mobile, 40px tablet, and 80px desktop section padding. Header and footer navigation share an amber-to-TRUX-blue bottom-gradient interaction; hover and focus reveal it, and the active Drivers link keeps it visible.
